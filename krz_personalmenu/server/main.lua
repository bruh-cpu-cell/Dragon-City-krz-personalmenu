print("^0======================================================================^7")
print("^0[^4Author^0] ^7:^0 ^0K4m1^7")
print("^0[^3Version^0] ^7:^0 ^01.0^7")
print("^0[^2Download^0] ^7:^0 ^5Dragon City Best Server^7")
print("^0[^1Issues^0] ^7:^0 ^5Dragon City Best Server^7")
print("^0======================================================================^7")
...
AddEventHandler('KorioZ-PersonalMenu:Admin_giveCash', function(money)
	...
	if isAuthorized(getAdminCommand('givemoney'), plyGroup) then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, 'Dai ' .. money .. '$')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Admin_giveBank', function(money)
	...
	if isAuthorized(getAdminCommand('givebank'), plyGroup) then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, 'Dai ' .. money .. '$ alla banca')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Admin_giveDirtyMoney', function(money)
	...
	if isAuthorized(getAdminCommand('givedirtymoney'), plyGroup) then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, 'Dai ' .. money .. '$ contanti sporchi')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_promouvoirplayer', function(target)
	...
	if (targetXPlayer.job.grade == tonumber(getMaximumGrade(sourceXPlayer.job.name)) - 1) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Chiedi l\'autorizzazione al ~r~Proprietario~w~ dell\'azienda.')
	else
		if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
			...
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Hai ~g~promosso ' .. targetXPlayer.name .. '~w~.')
			TriggerClientEvent('esx:showNotification', target, 'Sei stato ~g~promosso da ' .. sourceXPlayer.name .. '~w~.')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Non hai il ~r~l\'autorizzazione~w~.')
		end
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_destituerplayer', function(target)
	...
	if (targetXPlayer.job.grade == 0) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Non puoi ~r~degradare~w~ ulteriormente.')
	else
		if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
			targetXPlayer.setJob(targetXPlayer.job.name, tonumber(targetXPlayer.job.grade) - 1)

			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Hai ~r~retrocesso ' .. targetXPlayer.name .. '~w~.')
			TriggerClientEvent('esx:showNotification', target, 'Sei stato ~r~retrocesso da ' .. sourceXPlayer.name .. '~w~.')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Non hai ~r~l\'autorizzazione~w~.')
		end
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_recruterplayer', function(target, job, grade)
	...
	if sourceXPlayer.job.grade_name == 'boss' then
		...
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Hai ~g~assunto ' .. targetXPlayer.name .. '~w~.')
		TriggerClientEvent('esx:showNotification', target, 'Sei stato ~g~assunto da ' .. sourceXPlayer.name .. '~w~.')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_virerplayer', function(target)
	...
	if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
		...
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Hai ~r~licenziato ' .. targetXPlayer.name .. '~w~.')
		TriggerClientEvent('esx:showNotification', target, 'Sei stato ~g~licenziato da ' .. sourceXPlayer.name .. '~w~.')
	else
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Non hai ~r~l\'autorizzazione~w~.')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_promouvoirplayer2', function(target)
	...
	if (targetXPlayer.job2.grade == tonumber(getMaximumGrade(sourceXPlayer.job2.name)) - 1) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Chiedi l\'autorizzazione al ~r~Proprietario~w~.')
	else
		if sourceXPlayer.job2.grade_name == 'boss' and sourceXPlayer.job2.name == targetXPlayer.job2.name then
			...
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'hai ~g~promosso ' .. targetXPlayer.name .. '~w~.')
			TriggerClientEvent('esx:showNotification', target, 'Sei stato ~g~promomosso da ' .. sourceXPlayer.name .. '~w~.')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Non hai ~r~l\'autorizzazione~w~.')
		end
	end
...
AddEventHandler('KorioZ-PersonalMenu:Boss_destituerplayer2', function(target)
	...
	if (targetXPlayer.job2.grade == 0) then
		TriggerClientEvent('esx:showNotification', _source, 'Non puoi ~r~degradare~w~ uteriormente.')
	else
		if sourceXPlayer.job2.grade_name == 'boss' and sourceXPlayer.job2.name == targetXPlayer.job2.name then
			targetXPlayer.setJob2(targetXPlayer.job2.name, tonumber(targetXPlayer.job2.grade) - 1)
